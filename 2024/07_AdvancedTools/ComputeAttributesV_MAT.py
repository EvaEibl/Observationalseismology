...
#    for i in range(ntr):
    ...
    int1 = np.trapezoid(cor_smooth[int(ilag_0):int(ilag_third)+1]/max_cor)
    int2 = np.trapezoid(cor_smooth[int(ilag_third):]/max_cor)
    int_ratio = int1 / int2
...
#        tr = Data
        ...
        ES[j] = np.log10(np.trapezoid(np.abs(hilbert(data_filt))))
...
#    for i in range(ntr):
    ...
    E1FFT = np.trapezoid(FFTsmooth_norm[0:int(npts/4)])
    E2FFT = np.trapezoid(FFTsmooth_norm[int(npts/4-1):int(2*npts/4)])
    E3FFT = np.trapezoid(FFTsmooth_norm[int(2*npts/4-1):int(3*npts/4)])
    E4FFT = np.trapezoid(FFTsmooth_norm[int(3*npts/4-1):int(npts)])
